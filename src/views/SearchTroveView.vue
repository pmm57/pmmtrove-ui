<script setup>
  ...
  let searchCounts = reactive ({nbrNew: 0 ,
                    nbrKnown: 0,
                    nbrLessRelevant: 0,
                    nbrIgnored: 0,
                    nbrToIgnore: 0,
  ...
  function outputSearch () {
    ...
    searchData.searchResults.forEach (element => {
      ...
      if (stateIndex == -1) { // Sort Insert
        var newState = {label: element.state,
                        nbrFound: 0};
        stateIndex = 0;
  ...
  function waitSearch() {
    ...
    if (!!window.EventSource) {
      var searchName = userData.troveUserId + ':searchNbr' + newSearch.searchId;
